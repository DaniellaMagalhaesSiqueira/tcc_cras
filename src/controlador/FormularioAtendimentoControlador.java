...
public class FormularioAtendimentoControlador implements Initializable{
    ...
    public void clicarTipo() {
    	...
    	if(tipo==AtendimentoTipo.C) {
    		comboDemanda.getSelectionModel().select(DemandaAtendimento.CAD);
    		comboDemanda.setDisable(true);
    	}else {
    		
    		comboDemanda.getSelectionModel().select(DemandaAtendimento.INFO);
    		comboDemanda.setDisable(false);
    	}
    }
