...
//	private Label lbOutro;
	...
	@FXML
	private TextField telNome1;
	@FXML
	private TextField telNome2;
	@FXML
	private TextField telNome3;
...
//	private Tecnico tecnico;
	...
	@FXML
	private ComboBox<RedeReferenciada> comboRede;
	private ObservableList<RedeReferenciada> obsRede;
	private RedeReferenciada rede;
	...
	@FXML
	void clicarRede(ActionEvent event) {
		rede = comboRede.getSelectionModel().getSelectedItem();
	}
...
//		PessoaDAO daop = new PessoaDAO();
//		List<Pessoa> pessoas = daop.obterCondicao(id.toString(), Integer.toString(idUnidade));
...
//		        Long id = Long.valueOf(selectionModel.getSelectedIndex());//pega o id da TableList
	...
	public void preencherFamilia() {
		...
		for(String chave: entidade.getTelefone().keySet()) {
			txtTel1.setText(entidade.getTelefone().size() < 1 ? "" : chave);
			txtTel2.setText(entidade.getTelefone().size() < 2 ? "" : chave);
			txtTel3.setText(entidade.getTelefone().size() < 3 ? "" : chave);
		}
		for(String nome: entidade.getTelefone().values()) {
			telNome1.setText(entidade.getTelefone().size() < 1 ? "" : nome);
			telNome2.setText(entidade.getTelefone().size() < 2 ? "" : nome);
			telNome3.setText(entidade.getTelefone().size() < 3 ? "" : nome);
		}
...
//		tecnico = comboTecnico.getSelectionModel().getSelectedItem();
		tecnico.setText(entidade.getTecnico()==null?"Não possui Técnico de Referência":entidade.getTecnico().getNome());
		
		comboRede.getSelectionModel().select(entidade.getRedeEncaminhou());
//		situacao = comboSituacao.getSelectionModel().getSelectedItem();
		situacao.setText(entidade.getSituacao()==null?"Atendida":entidade.getSituacao().toString());
...
//		comboTecnico.getItems().add(null);
		...
		RedeDAO daor = new RedeDAO();
		List<RedeReferenciada> redes = daor.obterTodos();
		obsRede = FXCollections.observableArrayList(redes);
		comboRede.setItems(obsRede);
...
//		Familia f = em.find(Familia.class, entidade.getId());
		FamiliaDAO daof = new FamiliaDAO();
		...
		if(!txtTel1.getText().isEmpty()) {
			...
			telefones.put(txtTel1.getText(), telNome1.getText());
		}
		if(!txtTel2.getText().isEmpty()) {
			
			telefones.put(txtTel2.getText(), telNome2.getText());
		}
		if(!txtTel3.getText().isEmpty()) {
			
			telefones.put(txtTel3.getText(), telNome3.getText());
		}
...
//		f.setValorMoradia(txtAlugada.getText().equals("") ? 0 : Double.valueOf(txtAlugada.getText()));
		...
		//String recado = "recado";
		//Cria um map com uma chave e um valor
		//f.setTelefone(Collections.singletonMap(recado,txtTel1.getText()));
		
//		f.setSituacao(situacao.getText());
//		f.setTecnico(tecnico);
		f.setRedeEncaminhou(rede);
